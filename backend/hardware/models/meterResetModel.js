...
Calibration.findById = (DRN, result) => {
  connection.query(
    `SELECT * FROM Reset WHERE DRN = ${DRN}`,
    (err, res) => {
      if (err) {
        result(err, null);
        return;
...
Calibration.getLastCalibrationUpdate= (DRN, result) => {
  const query = "SELECT * FROM Reset WHERE ID = (SELECT MAX(ID) FROM Reset WHERE DRN = ?)";
...
Calibration.updatePrecessed = (DRN, processed,ID,result) => {
  connection.query(
    "UPDATE Reset SET processed = ? WHERE DRN = ? AND ID = ?",
    [processed,DRN,ID],
    (err, res) => {
      if (err) {
        result(null, err);
